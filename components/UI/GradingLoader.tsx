...
export const GradingLoader: React.FC<GradingLoaderProps> = ({ className = '' }) => {
  ...
  return (
    <div
      className={`rounded-lg bg-roman-50 border border-roman-200 p-8 text-center space-y-4 animate-fade-in ${className}`}
    >
      {/* Animated dots */}
      <div className="flex justify-center space-x-2">
        {[0, 1, 2].map((i) => (
          <div
            key={i}
            className="w-3 h-3 bg-pompeii-500 rounded-full animate-bounce"
      ...
      <div className="transition-opacity duration-300">
        <p className="text-lg font-serif text-roman-700">
